...
int main(int argc, char* argv[]){
    ...
    while(1){
        ...
        if(segment_size > 0){
            ...
            //printf("ipfrom: %s\n", ipfrom);
            //printf("portfrom: %d\n", portfrom);
            if(strcmp(ipfrom, ip[0]) == 0 && portfrom == port[0]) { //receive data from server
                ...
                if(s_tmp.head.fin == 1) { //connection ending
                    ...
                    if (rand() % 100 < 100 * loss_rate){ //mean drop
                        drop_data++;
            ...
            } else if(strcmp(ipfrom,ip[2]) == 0 && portfrom == port[2]) { //receive ack from receiver
